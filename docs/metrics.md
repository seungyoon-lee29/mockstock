...
> **리그 분리(ADR-0004)**: 시즌이 US·KR 리그별 독립이므로 ①② 지표는 리그별로 측정한다. `:kr_season` / `:us_season` 을 각 리그의 `season_id`로 치환할 것.
...
※ **리그별 측정**: KR 리그·US 리그 각각 아래 쿼리를 실행하고 결과를 병기한다.

```sql
-- KR 리그 (market = 'KR')
SELECT round(100.0 * count(DISTINCT o.user_id) / nullif(count(DISTINCT u.id), 0), 1) AS pct
FROM users u
LEFT JOIN orders o ON o.user_id = u.id AND o.status = 'filled' AND o.market = 'KR'
WHERE u.is_bot = false AND u.is_anonymous = false;

-- US 리그 (market = 'US')
SELECT round(100.0 * count(DISTINCT o.user_id) / nullif(count(DISTINCT u.id), 0), 1) AS pct
FROM users u
LEFT JOIN orders o ON o.user_id = u.id AND o.status = 'filled' AND o.market = 'US'
WHERE u.is_bot = false AND u.is_anonymous = false;
```
...
※ **리그별 측정**: `:prev_season`·`:cur_season`은 같은 리그(KR 또는 US)의 연속 시즌 id로 치환한다. 교차 리그(KR→US)는 의미 없으므로 측정하지 않는다.

```sql
-- KR 리그 예시 (:prev_season = 'season_...:KR', :cur_season = 'season_...:KR')
...
※ 리플레이는 US·KR 리그와 독립된 개인 훈련 모드이므로 리그별 분리 없이 통합 측정한다.
