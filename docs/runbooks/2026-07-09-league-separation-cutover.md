...
> **Railway GitHub 자동 배포 시 주의:** main 브랜치 push(3단계) 즉시 신 워커가 자동 기동될 수 있다. 마이그레이션(2단계)이 push보다 앞서 완료돼 있으면 신 스키마와 정합이 유지되므로 문제없다 — 단계 4는 자동 수행된 것으로 간주하고 부팅 스윕 로그만 확인한다.
