...
async function fetchChart(symbol: string, range: string): Promise<YahooResult | null> {
  ...
  try {
    ...
    return null; // 타임아웃·네트워크·파싱 실패 → fail-soft
  } finally {
    clearTimeout(timer);
  }
}
...
/** 지수 일봉(range 기본 1y) — 날짜 오름차순 DailyCandle[]. v=0(지수 거래량 무의미). 실패 시 []. */
export async function fetchYahooIndexDaily(symbol: string, range = "1y"): Promise<DailyCandle[]> {
  ...
  for (let i = 0; i < ts.length; i++) {
    ...
    out.push({ date: etDate(ts[i] * 1_000), o, h, l, c, v: 0 });
  }
  return out; // Yahoo는 이미 오름차순
}
