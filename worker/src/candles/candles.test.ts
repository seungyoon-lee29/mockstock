...
test("fetchKrDaily — EGW00201(HTTP 500) 1회 후 200이면 재시도로 성공", async () => {
  setKisEnv();
  process.env.KIS_RETRY_WAIT_MS = "1"; // 실대기 1.1초 회피
  let dataCalls = 0;
  try {
    await withFetch(
      (url) => {
        ...
        dataCalls++;
        if (dataCalls === 1)
          return new Response(JSON.stringify({ rt_cd: "1", msg_cd: "EGW00201", msg1: "초당 거래건수를 초과하였습니다." }), { status: 500 });
        return jsonRes({
          rt_cd: "0",
          output2: [{ stck_bsop_date: "20260710", stck_oprc: "70000", stck_hgpr: "71500", stck_lwpr: "69500", stck_clpr: "71000", acml_vol: "123456" }],
        });
      },
      async () => {
        const out = await fetchKrDaily("005930", "20260709", "20260710");
        assert.equal(dataCalls, 2); // 500 → 재시도 → 200
        assert.deepEqual(out, [{ date: "2026-07-10", o: 70000, h: 71500, l: 69500, c: 71000, v: 123456 }]);
      },
    );
  } finally {
    delete process.env.KIS_RETRY_WAIT_MS;
    clearKisEnv();
  }
});

test("fetchKrDaily — EGW00201 2회 연속이면 msg_cd 포함 에러", async () => {
  setKisEnv();
  process.env.KIS_RETRY_WAIT_MS = "1";
  let dataCalls = 0;
  try {
    await withFetch(
      (url) => {
      ...
      async () => {
        await assert.rejects(fetchKrDaily("005930", "20260709", "20260710"), /HTTP 500 EGW00201/);
        assert.equal(dataCalls, 2); // 재시도는 1회만
      },
    );
  } finally {
    delete process.env.KIS_RETRY_WAIT_MS;
...
test("fetchKrDaily — EGW00201 아닌 HTTP 500은 재시도 없이 즉시 throw", async () => {
  setKisEnv();
  process.env.KIS_RETRY_WAIT_MS = "1";
  let dataCalls = 0;
  try {
    await withFetch(
      (url) => {
        if (url.pathname.endsWith("/oauth2/tokenP")) return jsonRes({ access_token: "tok" });
        dataCalls++;
        return new Response("Internal Server Error", { status: 500 }); // JSON 아님 — msg_cd 파싱 불가
      },
      async () => {
        await assert.rejects(fetchKrDaily("005930", "20260709", "20260710"), /HTTP 500/);
        assert.equal(dataCalls, 1); // EGW00201 한정 재시도 — 그 외 500은 즉시 실패
      },
    );
  } finally {
    delete process.env.KIS_RETRY_WAIT_MS;
