...
type KisIndexOutput = {
  bstp_nmix_prpr?: string; // 업종지수 현재가
  bstp_nmix_prdy_vrss?: string; // 전일 대비
  bstp_nmix_prdy_ctrt?: string; // 전일 대비율(%) — 지수 TR 필드명(종목 prdy_ctrt와 구분)
  prdy_ctrt?: string; // 일부 TR 변형 대비 방어 파싱
};

/** 인덱스 폴러가 쓰는 파싱 결과. 값 미확보(비수치)면 null. */
export type KisIndexQuote = { value: number; change: number; changePct: number };

/**
 * 국내업종 현재지수(FHPUP02100000) — isuCd=0001(코스피)/1001(코스닥) 등.
 * 캔들 백필과 **동일한 tokenP·throttle 경로**(kisGet)를 타므로 KIS 초당 2건 상한을 함께 지킨다.
 * 키 없으면 null(호출부 fail-soft). 값 파싱 실패도 null.
 */
export async function fetchKrIndex(isuCd: string): Promise<KisIndexQuote | null> {
  if (!isKisRestEnabled()) return null;
  const json = await kisGet(INDEX_PATH, "FHPUP02100000", {
    FID_COND_MRKT_DIV_CODE: "U", // 업종
    FID_INPUT_ISCD: isuCd,
  });
  const o = (json.output ?? {}) as KisIndexOutput;
  const value = Number(o.bstp_nmix_prpr);
  const change = Number(o.bstp_nmix_prdy_vrss);
  const changePct = Number(o.bstp_nmix_prdy_ctrt ?? o.prdy_ctrt);
  if (![value, change, changePct].every(Number.isFinite)) return null;
...
/**
 * 상장주식수(느린 펀더멘털) — 주식현재가 시세(FHKST01010100)의 output.lstn_stcn(상장주수).
 * 반환은 정수 문자열(numeric(20,0) 왕복, float 금지·db.md) 또는 파싱 실패/키 부재 시 null.
 * 모든 KIS 호출과 동일한 kisGet(단일 throttle) 경유 — 2 req/s 불변식 공유(백필·인덱스 폴링과 함께).
 */
export async function fetchKrShares(symbol: string): Promise<string | null> {
  if (!isKisRestEnabled()) return null;
  const json = await kisGet(PRICE_PATH, "FHKST01010100", {
    FID_COND_MRKT_DIV_CODE: "J",
    FID_INPUT_ISCD: symbol,
  });
...
export function parseShares(raw: unknown): string | null {
  if (raw == null) return null;
  const digits = String(raw).replace(/[,\s]/g, "");
  if (!/^\d+$/.test(digits)) return null;
  const n = BigInt(digits); // 수십억 주 — Number 정밀도 밖일 수 있어 BigInt로 정규화
