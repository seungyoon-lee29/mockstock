...
## 작업 규칙 (승윤 지시, 2026-07-05)

- **메인 모델(Fable)은 판단만 한다** — 리뷰·조사·구현·문서 수정 등 생산 작업은 전부 서브에이전트에게 위임. 메인 모델이 직접 작업해야 하는 상황이면 반드시 사용자 허락을 먼저 받는다. 허락 없이는 절대 직접 작업 금지.
- 서브에이전트는 작업 시작 전 사용 가능한 스킬을 확인하고 적합한 것을 활용한다.
