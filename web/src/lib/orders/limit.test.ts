...
function place(
  db: DB,
  o: { market: "US" | "KR"; symbol: string; side: "buy" | "sell"; qty: number; limitPrice: number; fxRate: number },
  userId = "u1",
) {
  const id = oid();
...
// ① 매수 접수 — 예상금액 예약 차감 + reservedKrw 기록 + open.
test("① 매수 지정가 접수: 예약 차감·reservedKrw 기록", async () => {
  const db = await newDb();
  const { id, r } = await place(db, { market: "KR", symbol: "005930", side: "buy", qty: 2, limitPrice: 74_000, fxRate: 1 });
  assert.deepEqual(r, { ok: true });
  assert.equal(await cashCents(db), (SEED - 148_000) * 100); // 74000×1×2 예약
  const o = (await getOrder(db, id))!;
  assert.equal(o.status, "open");
  assert.equal(o.type, "limit");
  assert.equal(cents(o.reservedKrw!), 148_000 * 100);
  assert.equal(Number(o.fxRate), 1); // KR 매수는 fxRate=1 기록
});

// ② US 매수 접수 — 접수 시점 fxRate 고정 예약(§6.6).
test("② US 매수 지정가: 접수 fxRate 고정 예약", async () => {
  const db = await newDb();
  const { id, r } = await place(db, { market: "US", symbol: "AAPL", side: "buy", qty: 2, limitPrice: 230, fxRate: 1350 });
  assert.deepEqual(r, { ok: true });
  assert.equal(await cashCents(db), (SEED - 230 * 1350 * 2) * 100); // 621,000 예약
  const o = (await getOrder(db, id))!;
  assert.equal(Number(o.fxRate), 1350);
  assert.equal(cents(o.reservedKrw!), 230 * 1350 * 2 * 100);
});
...
test("④ 40% 상한 초과 접수: over-limit", async () => {
  const db = await newDb();
  const { id, r } = await place(db, { market: "KR", symbol: "005930", side: "buy", qty: 45, limitPrice: 100_000, fxRate: 1 }); // 450만 > 400만
  assert.deepEqual(r, { ok: false, reason: "over-limit" });
  assert.equal(await cashCents(db), SEED * 100); // 차감 없음
  assert.equal(await getOrder(db, id), null);
});
...
test("⑤ 대기 매수 예약 합산 40% 재검증", async () => {
  const db = await newDb();
  const a = await place(db, { market: "KR", symbol: "005930", side: "buy", qty: 30, limitPrice: 100_000, fxRate: 1 }); // 300만 예약
  assert.deepEqual(a.r, { ok: true });
  const b = await place(db, { market: "KR", symbol: "005930", side: "buy", qty: 20, limitPrice: 100_000, fxRate: 1 }); // +200만 → 500만 > 400만
  assert.deepEqual(b.r, { ok: false, reason: "over-limit" });
});

// ⑥ 매도 접수 — 보유 충분, 예약 컬럼 없음(reservedKrw/fxRate null).
test("⑥ 매도 지정가 접수: 보유 검증·예약 없음", async () => {
  const db = await newDb();
  await db.insert(positions).values({ userId: "u1", seasonId: "s1", market: "KR", symbol: "005930", qty: "5", costBasisKrw: "350000.00" });
  const { id, r } = await place(db, { market: "KR", symbol: "005930", side: "sell", qty: 3, limitPrice: 80_000, fxRate: 1 });
  assert.deepEqual(r, { ok: true });
  const o = (await getOrder(db, id))!;
  assert.equal(o.status, "open");
  assert.equal(o.reservedKrw, null);
  assert.equal(o.fxRate, null); // 매도는 체결 시점 환율(§6.6)
  assert.equal(await cashCents(db), SEED * 100); // 매도는 현금 변화 없음
});
...
test("⑦ 매도 보유 부족: open 매도 합산 insufficient-qty", async () => {
  const db = await newDb();
  await db.insert(positions).values({ userId: "u1", seasonId: "s1", market: "KR", symbol: "005930", qty: "5", costBasisKrw: "350000.00" });
  const first = await place(db, { market: "KR", symbol: "005930", side: "sell", qty: 4, limitPrice: 80_000, fxRate: 1 });
  assert.deepEqual(first.r, { ok: true });
  const second = await place(db, { market: "KR", symbol: "005930", side: "sell", qty: 3, limitPrice: 80_000, fxRate: 1 }); // 5 − 4 = 1 < 3
...
test("⑧ 매수 지정가 취소: 예약 환불·cancelled", async () => {
  const db = await newDb();
  const { id } = await place(db, { market: "KR", symbol: "005930", side: "buy", qty: 2, limitPrice: 74_000, fxRate: 1 });
  assert.equal(await cashCents(db), (SEED - 148_000) * 100);
  const c = await cancelOrder(db, "u1", id);
  assert.deepEqual(c, { ok: true, refunded: true });
  assert.equal(await cashCents(db), SEED * 100); // 환불
...
test("⑨ 매도 지정가 취소: refunded:false", async () => {
  const db = await newDb();
  await db.insert(positions).values({ userId: "u1", seasonId: "s1", market: "KR", symbol: "005930", qty: "5", costBasisKrw: "350000.00" });
  const { id } = await place(db, { market: "KR", symbol: "005930", side: "sell", qty: 3, limitPrice: 80_000, fxRate: 1 });
  const c = await cancelOrder(db, "u1", id);
...
test("⑩ 취소 방어: 타인·재취소는 ok:false, 환불 없음", async () => {
  ...
  assert.equal(await cashCents(db), SEED * 100); // 환불 1회만
});
