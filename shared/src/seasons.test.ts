...
const CFG = { market: "KR" as const, seedMoney: SEED, durationMs: 60_000 }; // 단축 시즌(1분 롤링)
...
test("③ reset 멱등: 신규 시즌 + 로스터 시드 재설정", async () => {
  ...
  await db.insert(seasons).values({ id: "sp", market: "KR", startsAt: new Date(Date.now() - 600_000), endsAt: new Date(Date.now() - 300_000), seedMoney: SEED.toFixed(2), status: "finalized" });
