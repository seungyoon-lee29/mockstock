...
const SEED = SEED_MONEY_KRW; // 1,000만
const CFG = { seedMoney: SEED, durationMs: 60_000 }; // 단축 시즌(1분 롤링)
...
async function newDb(): Promise<DB> {
  ...
  await db.insert(fxRates).values({ pair: "USDKRW", rate: "1350", fetchedAt: new Date() });
  return db;
}

let seq = 0;
async function acct(db: DB, seasonId: string, userId: string, cash = SEED, isBot = false) {
  await db.insert(users).values({ id: userId, name: userId, isBot }).onConflictDoNothing();
  await db.insert(accounts).values({ userId, seasonId, cashKrw: cash.toFixed(2) });
}
/** 시장가 매수를 fillOrder로 체결(현금 차감·포지션 적립). */
async function buy(db: DB, seasonId: string, userId: string, market: "US" | "KR", symbol: string, qty: number, price: number, fx = 1) {
  const id = `o${++seq}`;
  await db.insert(orders).values({ id, userId, seasonId, market, symbol, side: "buy", type: "market", qty: String(qty), idempotencyKey: id });
  await fillOrder(db, { orderId: id, userId, seasonId, market, symbol, side: "buy", orderType: "market", qty, filledPrice: price, fxRate: fx });
}
/** open 매수 지정가(예약 현금)를 삽입 + 계좌 현금 선차감(접수 시뮬레이션). 만료 스윕이 환불한다. */
async function reserveKrw(db: DB, seasonId: string, userId: string, reservedKrw: number) {
  const id = `o${++seq}`;
  await db.insert(orders).values({ id, userId, seasonId, market: "KR", symbol: "005930", side: "buy", type: "limit", qty: "1", limitPrice: "1", reservedKrw: reservedKrw.toFixed(2), status: "open", idempotencyKey: id });
  await db.update(accounts).set({ cashKrw: sql`${accounts.cashKrw} - ${reservedKrw.toFixed(2)}::numeric` }).where(and(eq(accounts.userId, userId), eq(accounts.seasonId, seasonId)));
  return id;
}
...
// ① 단축 시즌 풀사이클 — 생성→매매→만료→finalize(expire·환불·finalValue·rank).
test("① 풀사이클: 생성→매매→만료→확정(랭킹·환불·봇 제외)", async () => {
  const db = await newDb();
  const season = await ensureActiveSeason(db, CFG);
  assert.equal(season.status, "active");

  await acct(db, season.id, "u1");
  await acct(db, season.id, "u2");
  await acct(db, season.id, "b1", SEED, true); // 봇

  await buy(db, season.id, "u1", "KR", "005930", 10, 100_000); // cash 9,000,000 / hold 10×120,000
  await reserveKrw(db, season.id, "u1", 500_000); // cash 8,500,000, open 예약
  await buy(db, season.id, "u2", "KR", "000660", 5, 200_000); // cash 9,000,000 / hold 5×180,000
  await reserveKrw(db, season.id, "b1", 300_000); // 봇도 예약(만료 환불 검증용)
  ...
  assert.equal(await cashOf(db, season.id, "b1"), SEED * 100);
...
test("② finalize 멱등: 재실행은 no-op(이중 환불·중복 랭킹 없음)", async () => {
  ...
  const season = await ensureActiveSeason(db, CFG);
  await acct(db, season.id, "u1");
  await reserveKrw(db, season.id, "u1", 500_000);
  await expireNow(db, season.id);
...
// ③ reset 멱등 — 신규 시즌 생성 + 직전 로스터 시드 재설정, 재실행해도 중복/재설정 흔들림 없음.
test("③ reset 멱등: 신규 시즌 + 로스터 시드 재설정", async () => {
  const db = await newDb();
  // 직전(확정된) 시즌 + 매매로 줄어든 계좌.
  await db.insert(seasons).values({ id: "sp", startsAt: new Date(Date.now() - 600_000), endsAt: new Date(Date.now() - 300_000), seedMoney: SEED.toFixed(2), status: "finalized" });
  await acct(db, "sp", "u1", 3_000_000);
  await buy(db, "sp", "u1", "KR", "005930", 1, 100_000);

  const s1 = await resetSeason(db, CFG);
  assert.notEqual(s1.id, "sp");
  assert.equal(await cashOf(db, s1.id, "u1"), SEED * 100); // 신규 시즌 시드 재설정
  const posNew = await db.select().from(positions).where(eq(positions.seasonId, s1.id));
  assert.equal(posNew.length, 0); // 클린 슬레이트

  const s2 = await resetSeason(db, CFG);
  assert.equal(s2.id, s1.id); // 같은 기간 → 같은 시즌(멱등)
  assert.equal(await cashOf(db, s1.id, "u1"), SEED * 100);
...
test("④ 취소·만료 경합: 취소된 주문은 이중 환불 불가", async () => {
  ...
  await db.insert(orders).values({ id: "ocx", userId: "u1", seasonId: season.id, market: "KR", symbol: "005930", side: "buy", type: "limit", qty: "1", limitPrice: "1", reservedKrw: (700_000).toFixed(2), status: "cancelled", idempotencyKey: "ocx" });
...
test("⑤ snapshotPortfolios: 예약 현금 포함 총자산 기록", async () => {
  ...
  await acct(db, season.id, "u1");
  await buy(db, season.id, "u1", "KR", "005930", 10, 100_000); // cash 9,000,000 / hold 10×120,000
  await reserveKrw(db, season.id, "u1", 500_000); // cash 8,500,000 + 예약 500,000
  ...
  assert.equal(cents(snap.totalValueKrw), 10_200_000 * 100);
});
...
test("⑥ 랭킹 타이브레이커: 동률 시 MDD 낮은 쪽 상위", async () => {
  ...
  const snap = (userId: string, date: string, v: number) => ({ userId, seasonId: season.id, date, totalValueKrw: v.toFixed(2) });
